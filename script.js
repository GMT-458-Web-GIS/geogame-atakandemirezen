...
// Harita başlatma
function initMap() {
    map = L.map('map').setView([20, 0], 2); // Harita başlangıcı, dünya haritası
...
function highlightCountry(coords, color) {
    if (lastMarker) {
        map.removeLayer(lastMarker); // Eski işaretçiyi kaldır
    }
    ...
    map.setView(coords, 3); // Haritayı bu ülkenin etrafına yakınlaştır
}
...
function newQuestion() {
    ...
    } while (askedCountries.includes(countriesList[randomIndex].name)); // Aynı ülke kontrolü
